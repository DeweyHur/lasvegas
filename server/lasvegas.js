...
export class LasVegas extends GameRoom
{
  ...
  constructor(server) {
    ...
    this.nextPlayer = () => {
      ...
      if (this.currentPlayer) {
        let nextPlayer = (this.players.findIndex(player => player == this.currentPlayer) + 1) % this.players.count;
      ...
      if (this.currentPlayer) {
        this.broadcast({ cmd: 'nextPlayer', player: this.currentPlayer, game: this.toJson() });
        this.onAllReady = this.roll;
      } else {
        this.broadcast({ cmd: 'result', game: this.toJson() });
  ...
  invest(id, message) {
    if (this.currentPlayer.id == id) {
      ...
      this.broadcast({ cmd: 'invested', player: this.currentPlayer, table: message.table, dices: funds, game: this.toJson() });
      this.onAllReady = this.nextPlayer;
    }  
  }
